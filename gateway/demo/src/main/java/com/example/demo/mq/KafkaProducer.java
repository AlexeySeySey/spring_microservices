...
public class KafkaProducer {
  ...
  public String request(String topic, Object request) {
    return request(topic, request, false);
  }

  public String request(String topic, Object request, boolean hasResponse) {
    ...
    while (true) {
      ...
      if (start < (System.currentTimeMillis() - Integer.parseInt(kafkaRequestTimeout) * 1000L)) {
        kafkaResponseContainer.removeResponse(token);
        throw new RuntimeException(Error.KAFKA_TIMEOUT.get());
      }
    }
    return response;
  }
